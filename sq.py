...
defphi[i] = phi[i] == 4 * xi["0", i] / (x["0"] * x["1"] * xi["3", i]) + 4 * xi["1", i] * x["1"] / (
    gpm.sqr(x["0"]) * xi["3", i]) + gpm.sqr(xi["2", i]) / (gpm.sqr(x["0"]) * gpm.sqr(xi["3", i]))
...
superquantile = gp.Model(m, name="superquantile", equations=[defphi], problem="emp")
